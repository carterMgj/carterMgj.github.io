...
![图片]
(https://raw.githubusercontent.com/carterMgj/blog_img/master/2016-12-03-Hctf-fuzzPwn/1.png)
另外主办方提示：
  1. 这道题没有必要爆破密码
  2. 发送的数据量不要超过0x100个
...
![图片]
(https://raw.githubusercontent.com/carterMgj/blog_img/master/2016-12-03-Hctf-fuzzPwn/2.png)
**推测：**此时存在栈溢出函数的EBP被覆盖为无效地址，导致函数返回时读取EBP内容无效，从而程序崩溃退出，所以未能继续执行输出'No password,no game'。所以确定要覆盖到返回地址之前，需要的payload长度为72。
...
![图片]
(https://raw.githubusercontent.com/carterMgj/blog_img/master/2016-12-03-Hctf-fuzzPwn/3.png)
 - 什么都没有收到的情况是因为：返回到了某个需要参数输入的函数，正在等待参数输入；
 - 打印出'No password,no game'的情况是因为：又返回到了主函数内，从新进行了一遍主函数。
...
![图片]
(https://raw.githubusercontent.com/carterMgj/blog_img/master/2016-12-03-Hctf-fuzzPwn/4.png)
...
![图片]
(https://raw.githubusercontent.com/carterMgj/blog_img/master/2016-12-03-Hctf-fuzzPwn/5.png)

#### 6. 现在我们知道了 0x4007c3是'pop rdi,ret'，0x400570是puts函数的plt表地址。组合起来也就可以完成任意内存泄露的效果。首先我们泄露程序的代码段内存（0x0400000-0x0401000，大小随意）和程序数据段内存（0x601000-0x602000，大小随意）
```python
from pwn import *
...
![图片]
(https://raw.githubusercontent.com/carterMgj/blog_img/master/2016-12-03-Hctf-fuzzPwn/6.png)
知道put_got表地址后，就可以从data.bin中查看0x18地址,即0x601018地址内容，此时就可以泄露puts函数的内存地址，从而可以泄露libc基址了。
![图片]
(https://raw.githubusercontent.com/carterMgj/blog_img/master/2016-12-03-Hctf-fuzzPwn/7.png)
